...
def current_user():
    if "admin_id" in session:
        con = db()
        u = con.execute("SELECT * FROM users WHERE id=? AND role='admin'", (session["admin_id"],)).fetchone()
        con.close()
        if u:
            return u
    if "user_id" not in session:
        return None
    con = db()
    u = con.execute("SELECT * FROM users WHERE id=? AND role='user'", (session["user_id"],)).fetchone()
    con.close()
    return u
...
def register():
    if request.method == "POST":
        ...
        con = db()
        try:
            con.execute("INSERT INTO users(name,username,password) VALUES(?,?,?)", (name, username, generate_password_hash(password)))
            con.commit()
...
def admin_login():
    if request.method == "POST":
        ...
        password = request.form.get("password","")
        con = db()
        admin = con.execute("SELECT * FROM users WHERE username=? AND role='admin'", (username,)).fetchone()
        con.close()
...
def dashboard():
    if not current_user():
        return redirect(url_for("login"))
    con = db()
    rows = con.execute("SELECT * FROM analyses WHERE user_id=? ORDER BY id DESC", (session["user_id"],)).fetchall()
    con.close()
...
def analyze():
    ...
    try:
        ...
        result["career_recommendations"] = career_recommendations(result["skills"], text)
        con = db()
        cur = con.execute("INSERT INTO analyses(user_id,filename,score,result_json) VALUES(?,?,?,?)", (session["user_id"], safe, result["score"], json.dumps(result)))
        con.commit()
...
def result(analysis_id):
    if not current_user():
        return redirect(url_for("login"))
    con = db()
    row = con.execute("SELECT * FROM analyses WHERE id=? AND user_id=?", (analysis_id, session["user_id"])).fetchone()
    con.close()
    if not row:
...
def admin_dashboard():
    if "admin_id" not in session:
        return redirect(url_for("admin_login"))
    con = db()
    users = con.execute("SELECT id,name,username,role,created_at FROM users ORDER BY id DESC").fetchall()
    analyses = con.execute("SELECT a.*, u.username FROM analyses a JOIN users u ON a.user_id=u.id ORDER BY a.id DESC").fetchall()
    con.close()
