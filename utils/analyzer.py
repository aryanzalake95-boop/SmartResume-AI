...
def analyze_resume(text):
    if not text or not text.strip(): raise ValueError("No readable text was extracted from the resume.")
    low = re.sub(r"\s+", " ", text.lower())
    ...
    section_status = {k: "Detected" if v else "Missing" for k, v in sections.items()}
    return {"score": score, "word_count": word_count, "skills": skills, "skill_count": len(skills),
            "sections": sections, "section_status": section_status,
